...
class ApiObject(object):
    ...
    # url = "https://mdt-test.qingzhoudata.com"
    logyyx = Logger(logging.ERROR, logging.DEBUG)
    ...
    def run(self):
        ...
        request = Request(method=self.method, url=self.url, params=self.params, data=self.data, headers=self.headers, json=self.json)
        prepare = s.prepare_request(request)
        resp = s.send(prepare)
        data = dump.dump_all(resp)
        # print(data)
        # self.logyyx.info(data)
        #
        # if resp.status_code ==200:
        #     print(unquote(data.decode('unicode-escape')))
        # else:
...
if __name__ == '__main__':
    import re
    print(re.findall(r'\d+', 'hello 42 I\'m a 32 string 30')) #['42', '32', '30']
    key = '这个女孩身高170厘米'
    print(re.findall('\d+', key)[0]) #170
    kk = re.compile(r'\d+')
    print(kk)
