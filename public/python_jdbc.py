...
def _jdbc_connect_jpype(jars, libs, *args, **kwargs):
    ...
    jd = jpype.JClass("connect_db_api")
    driver_ls = jd.dynamicLoadJdbc(kwargs.get("jar_path"),
                                   kwargs.get("driver_name"), )
    try:
        if kwargs.get("iskrbs"):
            conn = jd.getKerberosConnect(kwargs.get("url"),
                                         kwargs.get("KrbRealm"),
                                         kwargs.get("ImpalaFQDN"),
                                         kwargs.get("KrbServiceName"),
                                         kwargs.get("KrbsKey"),
                                         kwargs.get("KrbsConfPath"),
                                         kwargs.get("KeytabPath"),
                                         kwargs.get("login_time_out"))
        else:
            conn = jd.testOneConnect(kwargs.get("url"), kwargs.get("user"),
                                     kwargs.get("password"),
                                     kwargs.get("jar_path"),
                                     kwargs.get("driver_name"),
                                     kwargs.get("login_time_out"))
        ...
        jd.dynamicUnLoadJdbc(driver_ls, kwargs.get("jar_path"), )
...
if sys.platform.lower().startswith('java'):
    raise EnvironmentError("环境信息不支持")
    # _prepare_jython()
else:
    _prepare_jpype()
...
class DBAPITypeObject(object):
    ...
    def _map_jdbc_type_to_dbapi(cls, jdbc_type_const):
        ...
        try:
            ...
            warnings.warn(
                "No type mapping for JDBC type '%s' (constant value %d). "
                "Using None as a default type_code." % (
                    type_name, jdbc_type_const))
            return None
...
def connect(jars=None, libs=None, *args, **kwargs):
    ...
    kwargs:
            url : "jdbc:mysql://192.168.202.1:3306/test?serverTimezone=UTC&useSSL=false"
            user : root
            password :mypassword
            jar_path:
            driver_name:
    ...
    if libs:
        ...
        libs = []
    jconn = _jdbc_connect(jars, libs, *args, **kwargs)
...
class Connection(object):
    ...
    def setautocommit(self, bool=True):
        self.jconn.setAutoCommit(bool)
...
class Cursor(object):
    ...
    def description(self):
        ...
        if m:
            ...
            for col in range(1, count + 1):
                ...
                jdbc_type_name = m.getColumnTypeName(col)
                if jdbc_type == 0:
                    # PEP-0249: SQL NULL values are represented by the
                    # Python None singleton
                    dbapi_type = None
                else:
                    dbapi_type = DBAPITypeObject._map_jdbc_type_to_dbapi(
                        jdbc_type)

                col_desc = (m.getColumnName(col),  # m.getColumnName(col)
                            jdbc_type_name,
                            dbapi_type,
                            size,
                            size,
    ...
    def execute(self, operation, parameters=None, query_time_out=None):
        # setQueryTimeout
        ...
        if query_time_out:
            self._prep.setQueryTimeout(query_time_out)
    ...
    def execute_by_createStatement(self, operation, parameters=None,
                                   query_time_out=None):
        # setQueryTimeout
        if self._connection._closed:
            raise Error()
    ...
    def fetchone(self):
        ...
        for col in range(1, self._meta.getColumnCount() + 1):
            ...
            if sqltype in [-101, -102]:
                v = _to_datetime(self._rs, col)
                if sqltype == (-101):
                    v = datetime.datetime.strptime(str(v), "%Y-%m-%d %H:%M:%S")
                    v = str(v + datetime.timedelta(hours=-8))
            row.append(v)
        return tuple(row)
...
def _to_datetime(rs, col):
    ...
    if not java_val:
        return
    simpleDateFormat  = jpype.java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
    d = simpleDateFormat.format(java_val)
...
def _to_unsupported(rs, col):
    java_val = rs.getObject(col)
    if java_val is None:
        return
    java_val = str(java_val)
    java_val = java_val.split("@")[0] if "@" in java_val else java_val
    return java_val
...
def _java_to_py(java_method):
    def to_py(rs, col):
        ...
        if PY2 and isinstance(java_val, (string_type, int, long, float, bool)):
            ...
            return java_val
        elif isinstance(java_val, bytes):

            print("java_val:", type(java_val))
...
# _to_clob=_java_to_py("clobValue")
...
_DEFAULT_CONVERTERS = {
    ...
    'BOOLEAN': _to_boolean,
    'BIT': _to_string,
    'BLOB': _to_unsupported,
    'CLOB': _to_unsupported,
    'ROWID': _to_unsupported,
    'NCLOB': _to_unsupported
